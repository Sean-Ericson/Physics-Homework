...
\section*{1.1.7}
The flaw in the induction step is obvious when there are two ducks. Let $n=1$ and $m=n+1=2$. We then consider the sets $\{1,2,\cdots,m\} = \{1\}$ and $\{2,...,m,m+1\}=\{2\}$ Each of these sets are themselevs the same color (trivially, and by the induction hypothesis). The problem is now with the claim "the ducks $\#2$ throuh $m$ are all the same color". This claim revers to the ducks ``shared'' between the two sets, but the sets are distinct so no elemets are shared between them.
