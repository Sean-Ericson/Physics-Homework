...
\section*{Problem 2}
\begin{enumerate}[label=(\alph*)]
    \item As we showed in class, $C(\hat{\alpha},\hat{\beta}) = -\hat{\alpha}\cdot\hat{\beta}$ for the Bell pair $\frac{1}{\sqrt{2}}(\ket{1, 0} - \ket{0, 1})$. For each pair in this case,
    \[ \hat{\alpha}\cdot\hat{\beta} = \cos(\frac{\pi}{4}) = \frac{1}{\sqrt{2}} \]
    The total correlation combination is then
    \[ \mathscr{C} = -\frac{3-1}{\sqrt{2}} = -\sqrt{2} \]
